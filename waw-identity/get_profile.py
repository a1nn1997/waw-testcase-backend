...
class ProfileUpdater:
    ...
    def update_profile(self, profile: identity_pb2.UserProfile) -> identity_pb2.UserProfile:
        """Update the profile and return the response."""
        try:
            return self.stub.UpdateProfile(identity_pb2.ProfileDelta(profile=profile))
        except grpc.RpcError as e:
            print(f"Error updating profile: {e.details()}")
            return identity_pb2.UserProfile()

def create_or_update_profile(existing: identity_pb2.UserProfile, args) -> identity_pb2.UserProfile:
    """Create or update a user profile."""
    now = datetime.now(timezone.utc).isoformat()  # Convert to ISO 8601 string
    ...
    profile = identity_pb2.UserProfile(
        ...
        email=args.email,
        phone=args.phone,
        created_at=existing.created_at or now,  # Ensure these are strings (ISO format)
        updated_at=now,  # Ensure this is also a string (ISO format)
    )
    return profile
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Update user profile via gRPC")
    parser.add_argument("--id", help="User ID (optional, autogenerated if not provided)")
    parser.add_argument("--name", required=True, help="User full name")
    parser.add_argument("--email", required=True, help="User email address")
    parser.add_argument("--phone", required=False, default="", help="Phone number")
